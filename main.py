...
def run_simulation(L, p_block, start, end, randomize_points, manual_blockades_enabled):
    ...
    def bfs(start, end, blocked_edges):
        ...
        while queue:
            ...
            if x < L: neighbors.append((x + 1, y))
            if y > 0: neighbors.append((x, y - 1))
            if y < L: neighbors.append((x, y + 1))
    ...
    while current_position != end:
        path = bfs(current_position, end, blocked_edges)
        if path is None or len(path) < 2:
            messagebox.showwarning("Brak trasy", "Nie znaleziono możliwej trasy do celu z powodu blokad.")
            visualize_failed_path(start, end, blocked_edges, L, current_position, actual_path)
            return
...
def start_gui():
    ...
    tk.Checkbutton(root, text="Losowe punkty start i koniec", variable=var_random, command=toggle_manual_entries).grid(row=2, columnspan=2)
